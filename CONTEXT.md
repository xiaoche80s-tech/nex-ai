...
**MCP Server（MCP 服务）**:
注册的外部 MCP 服务连接（端点/传输/认证），其工具经探测同步进 MCP 工具注册表，供智能体挂载。
_Avoid_: MCP 接入、工具服务

**MCP 工具注册表（MCP Tool Registry）**:
MCP Server 探测同步落库的工具清单，每行一个工具的快照（名称/描述/参数模式）与治理标记（敏感/启用）；启用标记即该 server 的工具白名单。平台内置工具不在此表。
_Avoid_: 工具库、平台工具库（后者指 @Tool 注册的平台工具）

**敏感工具（Sensitive Tool）**:
调用前须人工审批（HITL）的工具，两级定性：注册表内的 server 级定性（全局默认）与挂载级加标，任一命中即审批，挂载级只能加严不能放宽。
_Avoid_: 危险工具、受控工具、readOnlyHint（外部注解，非定性来源）
