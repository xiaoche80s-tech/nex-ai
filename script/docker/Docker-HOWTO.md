...
依赖：docker compose v2，删除`name: nexai-system`，降低`version`版本为`3.3`以下，支持`docker-compose`。
...
```shell
# 创建maven缓存volume
docker volume create --name nexai-maven-repo

docker run -it --rm --name nexai-maven \
    -v nexai-maven-repo:/root/.m2 \
