package com.gkht.ai.nexai.module.infra.service.codegen.inner;
...
import com.gkht.ai.nexai.framework.apilog.core.annotation.ApiAccessLog;
import com.gkht.ai.nexai.framework.apilog.core.enums.OperateTypeEnum;
...
import com.gkht.ai.nexai.framework.common.util.date.DateUtils;
import com.gkht.ai.nexai.framework.common.util.date.LocalDateTimeUtils;
import com.gkht.ai.nexai.framework.common.util.object.BeanUtils;
import com.gkht.ai.nexai.framework.common.util.object.ObjectUtils;
import com.gkht.ai.nexai.framework.common.util.string.StrUtils;
import com.gkht.ai.nexai.framework.excel.core.annotations.DictFormat;
import com.gkht.ai.nexai.framework.excel.core.convert.DictConvert;
import com.gkht.ai.nexai.framework.excel.core.util.ExcelUtils;
import com.gkht.ai.nexai.framework.mybatis.core.dataobject.BaseDO;
...
import com.gkht.ai.nexai.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.gkht.ai.nexai.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.gkht.ai.nexai.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.gkht.ai.nexai.module.infra.enums.codegen.CodegenSceneEnum;
import com.gkht.ai.nexai.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.gkht.ai.nexai.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.gkht.ai.nexai.module.infra.framework.codegen.config.CodegenProperties;
...
import static com.gkht.ai.nexai.module.infra.enums.ErrorCodeConstants.CODEGEN_MASTER_TABLE_FIELD_DUPLICATE;
import static com.gkht.ai.nexai.module.infra.enums.ErrorCodeConstants.CODEGEN_MASTER_TABLE_NAME_DUPLICATE;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 nexai-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 nexai-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.gkht.ai.nexai.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/nexai-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/nexai-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "nexai-module-${table.moduleName}/" + // 顶级模块
                "nexai-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "nexai-module-${table.moduleName}/" + // 顶级模块
                "nexai-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "nexai-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "nexai-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "nexai-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "nexai-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
