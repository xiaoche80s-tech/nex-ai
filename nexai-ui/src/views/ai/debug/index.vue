  ...
  >
    ...
    <div class="w-260px flex flex-col bg-[var(--app-content-bg-color)] rounded-4px">
      <div class="p-12px flex items-center justify-between">
        ...
        <el-button type="primary" size="small" @click="createDialogRef?.open()" v-hasPermi="['ai:session:create']">
      ...
      <el-scrollbar class="flex-1">
        <div
          ...
          :key="session.id"
          class="mx-8px mb-6px px-10px py-8px rounded-4px cursor-pointer hover:bg-[var(--el-fill-color-light)]"
          :class="{ 'bg-[var(--el-color-primary-light-9)]': activeSession?.id === session.id }"
          @click="openSession(session)"
        >
          <div class="text-13px truncate">{{ session.title || `会话 #${session.id}` }}</div>
          <div class="text-12px text-[var(--el-text-color-secondary)] mt-2px flex items-center justify-between">
            <span>规格 #{{ session.specId }} · v{{ session.versionNo }}</span>
            <span>{{ session.messageRounds }} 轮</span>
          </div>
        </div>
        <el-empty v-if="!sessions.length && !loadingSessions" description="暂无调试会话" :image-size="60" />
      </el-scrollbar>
    </div>
    ...
    <div class="flex-1 min-w-0 flex flex-col bg-[var(--app-content-bg-color)] rounded-4px">
      <template v-if="activeSession">
        <div class="px-16px py-10px border-b border-[var(--el-border-color-lighter)] flex items-center justify-between">
          <span class="font-bold truncate">{{ activeSession.title || `会话 #${activeSession.id}` }}</span>
          <el-tag size="small" type="info" disable-transitions v-if="streaming">事件流接收中…</el-tag>
        </div>
        ...
        <el-scrollbar ref="chatScrollbarRef" class="flex-1 px-16px py-12px">
          <template v-for="(entry, index) in entries" :key="index">
            ...
            <div v-if="'kind' in entry" class="flex justify-end mb-16px">
              <div class="max-w-70% bg-[var(--el-color-primary)] text-white text-14px leading-22px px-12px py-8px rounded-8px whitespace-pre-wrap break-words">
                {{ entry.text }}
              </div>
            </div>
            <!-- 助手回合：思考块 / 工具卡片 / 正文 / 错误 -->
            <div v-else class="mb-20px">
              <el-collapse v-if="entry.thinking" class="mb-8px">
                <el-collapse-item title="思考过程" name="thinking">
                  <div class="text-13px text-[var(--el-text-color-secondary)] whitespace-pre-wrap">{{ entry.thinking }}</div>
                </el-collapse-item>
              </el-collapse>
              <div v-for="tool in entry.tools" :key="tool.toolCallId" class="mb-8px">
                <div class="border border-[var(--el-border-color)] rounded-6px text-13px overflow-hidden">
                  <div class="px-10px py-6px bg-[var(--el-fill-color-light)] flex items-center gap-6px">
                    <Icon :icon="tool.state === 'error' ? 'ep:circle-close-filled' : 'ep:setting'" :color="tool.state === 'error' ? 'var(--el-color-danger)' : 'var(--el-color-primary)'" />
                    <span class="font-bold">工具调用 · {{ tool.name }}</span>
                    <el-tag size="small" :type="tool.state === 'running' ? 'warning' : tool.state === 'error' ? 'danger' : 'success'" disable-transitions>
                      {{ tool.state === 'running' ? '执行中' : tool.state === 'error' ? '失败' : '完成' }}
                    </el-tag>
                  </div>
                  <div v-if="tool.resultText" class="px-10px py-6px whitespace-pre-wrap break-words text-[var(--el-text-color-regular)] max-h-160px overflow-auto">
                    {{ tool.resultText }}
                  </div>
                </div>
              </div>
              <div v-if="entry.text" class="text-14px leading-22px whitespace-pre-wrap break-words">{{ entry.text }}</div>
              <div v-if="entry.error" class="text-13px text-[var(--el-color-danger)]">运行出错：{{ entry.error }}</div>
              <div v-else-if="!entry.finished" class="text-13px text-[var(--el-text-color-secondary)]">
        ...
        <div class="px-16px py-12px border-t border-[var(--el-border-color-lighter)]">
          <div class="flex items-end gap-8px">
            ...
            />
            <el-button type="primary" :loading="streaming" :disabled="!inputText.trim()" @click="handleSend" v-hasPermi="['ai:session:message']">
              发送
            </el-button>
          </div>
        </div>
    ...
    <div class="w-300px flex flex-col bg-[var(--app-content-bg-color)] rounded-4px overflow-auto">
      <div class="px-14px py-10px font-bold border-b border-[var(--el-border-color-lighter)]">运行详情</div>
      <template v-if="activeSession">
        <el-descriptions :column="1" size="small" class="px-14px py-10px" label-class-name="!w-84px">
          <el-descriptions-item label="规格">#{{ activeSession.specId }}</el-descriptions-item>
          <el-descriptions-item label="绑定版本">
            <el-tag size="small" type="success" disable-transitions>v{{ activeSession.versionNo }}</el-tag>
            <span v-if="detailVersion?.remark" class="ml-4px text-12px">{{ detailVersion.remark }}</span>
          ...
          <el-descriptions-item label="maxIters">{{ detailConfig?.maxIters ?? '默认' }}</el-descriptions-item>
          <el-descriptions-item label="温度">{{ detailConfig?.temperature ?? '默认' }}</el-descriptions-item>
          <el-descriptions-item label="已发消息">{{ activeSession.messageRounds }} 轮</el-descriptions-item>
        ...
        <div class="px-14px py-8px">
          <el-collapse>
            <el-collapse-item :title="`系统提示${detailConfig?.systemPrompt ? '' : '（空）'}`" name="prompt">
              <div v-if="detailConfig?.systemPrompt" class="text-12px leading-20px whitespace-pre-wrap max-h-200px overflow-auto">
...
const handleSend = async () => {
  ...
  try {
    ...
    failTurn(error?.message?.includes('Failed to fetch') ? '连接中断，请重试' : error?.message ?? '事件流异常')
